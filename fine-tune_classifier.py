...
def construct_model(image_width, image_height):
    ...
    weights_location = os.path.join(
        runs_dir, model_name)  # or model_name_complete
...
# this is the augmentation configuration we will use for the trianing data
# (data augmentation)
...
checkpointer2 = ModelCheckpoint(filepath=save_model_name2, monitor='val_loss',
...
def fine_tuning():
    ...
    generators = construct_tuning_generators(
        img_width, img_height, train_data_dir, validation_data_dir)
...
# "train_classifier.ipynb"). If we are not using Jupyter, run the
# following modules.
if not Jupyter:
