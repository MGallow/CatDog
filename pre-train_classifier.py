...
#construct our neural network from the "VGG16.py" script. See Keras documentation for more information.
...
#this is the augmentation configuration (only rescaling) we will use for testing our classifier. Note that there is no need to create a configuration for the training data since the model is already pre-trained.
...
def bottleneck_features():
    ...
    generator_train = construct_image_generators(img_width, img_height, train_data_dir)
    generator_validation = construct_image_generators(img_width, img_height, validation_data_dir)
    ...
    bottleneck_features_validation = model.predict_generator(generator_validation, nb_validation_samples)
    np.save(open(validation_bottleneck, 'wb'), bottleneck_features_validation)


#we create the fully-connected model that will be stacked on top of the pre-trained model
...
def train_top_model():
    ...
    validation_data = np.load(open(runs_dir + "/bottleneck_VGG16_validation.npy", 'rb'))
...
#optionally, we can run the file from within Jupyter (see "pre-train_classifier.ipynb"). If we are not using Jupyter, run the following modules.
if not Jupyter:
    if Pre_train:
        bottleneck_features()
